...
{
public:
    ~ForceController() {sub_q_cmd_.shutdown(); sub_forcetorque_sensor_.shutdown();}
    ...
    bool init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i=0; i<n_joints_; i++)
        {
            ...
            if ( n.getParam("/elfin/force_controller/joint" + si + "/tdc/mbar", Mbar[i]) )
            {
                Mbar_(i) = Mbar[i];
            }
            else
            {
                std::cout << "/elfin/force_controller/joint" + si + "/tdc/mbar" << std::endl;
                ROS_ERROR("Cannot find tdc/mbar gain");
                return false;
            }

            if ( n.getParam("/elfin/force_controller/joint" + si + "/tdc/r", Ramda[i]) )
            {
                Ramda_(i) = Ramda[i];
            }
            else
            ...
            if ( n.getParam("/elfin/force_controller/joint" + si + "/tdc/a", Alpha[i]) )
            {
                Alpha_(i) = Alpha[i];
            }
            else
            ...
            if ( n.getParam("/elfin/force_controller/joint" + si + "/tdc/w", Omega[i]) )
            {
                Omega_(i) = Omega[i];
            }
            else
        ...
        if (!n.getParam("/elfin/force_controller/aic/fd", Fd_temp_))
        {
            ROS_ERROR("Cannot find aci/fd");
            return false;
        }

        if (!n.getParam("/elfin/force_controller/aic/m", M_))
        {
            ROS_ERROR("Cannot find aci/m");
            return false;
        }

        if (!n.getParam("/elfin/force_controller/aic/b", B_))
        {
            ROS_ERROR("Cannot find aci/b");
            return false;
        }

        if (!n.getParam("/elfin/force_controller/mode", experiment_mode_))
        {
        ...
        // command
        sub_q_cmd_ = n.subscribe("command", 1, &ForceController::commandCB, this);
        sub_forcetorque_sensor_ = n.subscribe<geometry_msgs::WrenchStamped>("/elfin/elfin/ft_sensor_topic", 1, &ForceController::updateFTsensor, this);
        ...
        for (size_t i = 0; i < nn_joints_; i++)
        {
            ROS_INFO("%d",i);
            std::string si = boost::lexical_cast<std::string>(i + 1);
            if (n.getParam("/elfin/force_controller/gains/elfin_joint" + si + "/pid/p", KKp[i]))
            {
                KKp_(i) = KKp[i];
            }
            else
            {
                std::cout << "/elfin/force_controller/gains/elfin_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/elfin/force_controller/gains/elfin_joint" + si + "/pid/i", KKi[i]))
            {
                KKi_(i) = KKi[i];
            }
            else
            ...
            if (n.getParam("/elfin/force_controller/gains/elfin_joint" + si + "/pid/d", KKd[i]))
            {
                KKd_(i) = KKd[i];
            }
            else
        ...
        if (cctr_obj_ == 1)
        {
            if (!n.getParam("/elfin/force_controller/clik_gain/K_regulation", KK_regulation_))
            {
        ...
        {
            if (!n.getParam("/elfin/force_controller/clik_gain/K_tracking", KK_tracking_))
            {
...
*/
        ...
        */
        ...
        k_stiff_damp_sub = n.subscribe<std_msgs::Float64MultiArray>("force_gains", 1, &ForceController::admit_gains, this);
        ...
        publisher_ = n.advertise<std_msgs::Float64MultiArray>("publish_force", 1000);
    ...
    void update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        if (total_time_ >= 28.0)
        {
           ...
           f_error += (f_d - f_e)*ddt;
           z = Kpfc*(f_d - f_e) + Kifc * f_error;

           //Works 2 lines
           // z = Kpfc*(f_d-f_e);
           // xxd_.p(2) = zz - z*0.1;

           // zdot = Kpfc*(f_d - f_e) * 0.01;
           // z=z_pre + zdot*ddt;
           // z_pre = xxd_.p(2);
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::ForceController, controller_interface::ControllerBase)
