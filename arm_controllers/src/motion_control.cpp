...
{
class motion_control : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
public:
        bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
        {
                ...
                for (size_t i = 0; i < n_joints_; i++)
                {
                        ...
                        if (n.getParam("/elfin/motion_control/gains/elfin_joint" + si + "/pid/p", Kp[i]))
                        {
                                Kp_(i) = Kp[i];
                        }
                        else
                        {
                                std::cout << "/elfin/motion_control/gains/elfin_joint" + si + "/pid/p" << std::endl;
                                ROS_ERROR("Cannot find pid/p gain");
                                return false;
                        }

                        if (n.getParam("/elfin/motion_control/gains/elfin_joint" + si + "/pid/i", Ki[i]))
                        {
                                Ki_(i) = Ki[i];
                        }
                        else
                        ...
                        if (n.getParam("/elfin/motion_control/gains/elfin_joint" + si + "/pid/d", Kd[i]))
                        {
                                Kd_(i) = Kd[i];
                        }
                        else
                ...
                if (ctr_obj_ == 1)
                {
                        if (!n.getParam("/elfin/motion_control/clik_gain/K_regulation", K_regulation_))
                        {
                ...
                {
                        if (!n.getParam("/elfin/motion_control/clik_gain/K_tracking", K_tracking_))
                        {
                ...
                // 6.2 subsriber
                sub_x_cmd_ = n.subscribe<std_msgs::Float64MultiArray>("command", 1, &motion_control::commandCB, this);
...
//                                xd_.M = x_.M;
                ...
                //aux_d_.data =M_.data*(J_inv_*(Kp_.data.cwiseProduct(ex_)));
                aux_d_.data =M_.data*(J_inv_*(xd_ddot_ + Kd_.data.cwiseProduct(ex_dot_)+Kp_.data.cwiseProduct(ex_)));
                comp_d_.data = C_.data + G_.data;
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::motion_control, controller_interface::ControllerBase)
